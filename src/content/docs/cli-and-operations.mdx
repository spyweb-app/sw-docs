...
Control log verbosity with `-q`/`-qq` or the `SPYWEB_LOG` environment variable:
...
SPYWEB_LOG=warn ./spyweb start
SPYWEB_LOG=error ./spyweb start

# Flags and other options can be combined freely:
...
SpyWeb includes a test runner for `test_*` functions defined in your Lua hooks and programmable API server (see [Lua Testing](/testing) for writing tests). Each test runs in an isolated Lua VM with a temporary database.
...
# Run tests for the programmable API server (auto-starts server)
./spyweb test server

# Run server tests matching a pattern
...
:::caution
`spyweb check` exits 0 even on config errors (output is purely informational). Use `spyweb check config` to validate — it exits 1 on errors.
:::
...
./spyweb update --check   # -c

# Skip version check, always download and replace
./spyweb update --force   # -f

# Keep old binary with custom suffix
./spyweb update --keep          # -k, spyweb-v{version}
./spyweb update --keep backup   # -k backup, spyweb-backup

# Discard old binary entirely
./spyweb update --overwrite   # -o
```
