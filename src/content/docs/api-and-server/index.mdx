...
| `/api/v/{name}` | Custom endpoints — requires auth (defined in `server/init.lua`) |
| `/api/public/{name}` | Custom endpoints — always public, no auth required (defined in `server/init.lua`) |

### Authentication

If `SPYWEB_API_KEY` is set, all `/api/*` endpoints require the `X-SpyWeb-Key` header — except `/api/public/*` routes, which are always accessible without a key:

```bash
curl -H "X-SpyWeb-Key: your_secret_key" http://127.0.0.1:7979/api/jobs
curl http://127.0.0.1:7979/api/public/health
```
...
Each HTTP request to `/api/v/{name}` (or `/api/public/{name}` for unauthenticated endpoints):
...
4. For `/api/v/{name}`, looks up `get["name"]` → `post["name"]` → `all["name"]`. For `/api/public/{name}`, looks up `public.get["name"]` → `public.post["name"]` → `public.all["name"]`
...
The method tables `get`, `post`, `put`, `patch`, `delete`, `all`, `public.get`, `public.post`, `public.put`, `public.patch`, `public.delete`, and `public.all` are pre-injected.
...
The server can also be configured via CLI flags on `spyweb start`:

| Flag | Short | Description |
|------|-------|-------------|
| `--port <PORT>` | | Customize the server port (default 7979) |
| `--no-server` | `-n` | Start without the web server |
| `--quiet` | `-q` | Suppress info logs (use `-qq` for errors only) |
...
**Endpoints return 404:** Verify the handler is in the correct method table for the URL prefix — `/api/v/{name}` uses `get.{name}`, `post.{name}`, etc.; `/api/public/{name}` uses `public.get.{name}`, `public.post.{name}`, etc. The `all` (or `public.all`) table is checked last.
