...
7. Drops the VM — no shared state between requests
...
|-------|------|-------------|
| `self.body` | string or nil | Raw request body (POST, PUT, PATCH) |
...
The timeout is enforced via Luau's bytecode interrupt hook — no threads are leaked.
