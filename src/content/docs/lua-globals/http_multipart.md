...
:::note
Async and yields the Lua VM. Default timeout 30s, max response body 10MB. File fields require `content` key. Field values must be strings or tables. Does not support proxy configuration.
:::
...
- [http_request](/lua-globals/http-request) — Arbitrary HTTP request with proxy support
